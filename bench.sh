...
global=$( wget -O /dev/null http://mirror.global-layer.com/speedtest/100mb.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
echo "Download speed from Global-Layer, NL: $global "
...
echo "Download speed from Linode, Newark, NJ: $newark "
...
echo "Download speed from Linode, Atlanta, GA: $atlanta "
dallas=$( wget -O /dev/null http://speedtest.dallas.linode.com/100MB-dallas.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
echo "Download speed from Linode, Dallas, TX: $dallas "
...
echo "Download speed from linnode, Fremont, Cali: $fremont "
tweak=$( wget -O /dev/null http://speedtest.tweak.nl/100mb.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
echo "Download speed from tweak, NL: $tweak "
swift=$( wget -O /dev/null http://speedtest.swiftway.net/100MB.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
echo "Download speed from Swiftway, NL: $swift "
serverius=$( wget -O /dev/null http://speedtest.serverius.com/1000mb.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
...
echo "Download speed from CacheFly, Chicago, IL: $cachefly "
...
gl=$(wget -O/dev/null http://mirror.global-layer.com/speedtest/1000mb.bin 2>&1 | awk '/\/dev\/null/ {speed=$3 $4} END {gsub(/\(|\)/,"",speed); print speed}' )
echo "Download speed from global-layer.com, NL: $gl"
